...

{
    ...
    [SerializeField] private List<BattleMove> moves = new List<BattleMove>();
    ...
    public bool HasMove(int moveIndex)

    {

        return moveIndex >= 0 && moveIndex < moves.Count && moves[moveIndex] != null;
...

    {
        ...
        return HasMove(moveIndex) && moves[moveIndex].HasUsesLeft;
...

    {
...

        {
            ...
            if (moves[i] != null && moves[i].HasUsesLeft)
...

    {
        ...
        return HasMove(moveIndex) ? moves[moveIndex] : null;

    }

    public int CalculateDamage(BattleMove move, BattleMonster target)
